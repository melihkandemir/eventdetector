function [out1, out2] = gpr_fnNew(logtheta, covfunc, x, y, A, fixed)
...
L = chol(K + diag(A))';                        % cholesky factorization of the covariance
alpha = solve_chol(L',y);

if nargin == 5
    fixed = zeros(length(logtheta),1);
end
...
if nargout == 2               % ... and if requested, its partial derivatives
  ...
  % do not compute derivative for the last parameter which is 
  % the noise in the likelihood; this derivative will be zero
  for i = 1:length(out2)
    if fixed(i) == 0   
